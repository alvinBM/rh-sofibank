...
import { FiPlus, FiSearch, FiMoreVertical, FiEye, FiEdit, FiShare2, FiUsers, FiTrash2, FiLock, FiCheckCircle } from "react-icons/fi";
import { useForm, Controller } from "react-hook-form";
import { toast } from "react-toastify";
import { useGetJobOpenings, useCreateJobOpening, useUpdateJobOpening, useDeleteJobOpening, usePublishJobOpening } from "@/src/hooks/useRecruitment";
...
export default function JobOpeningsPage() {
    ...
    const filteredServices = servicesData?.services?.filter((service) => service.direction_id === selectedDirection) || [];
    ...
    const filteredJobs = jobOpenings?.filter((job) => searchQuery === "" || job.title?.toLowerCase().includes(searchQuery.toLowerCase()) || job.job_number?.toLowerCase().includes(searchQuery.toLowerCase())) || [];
        ...
        <PermissionGuard requiredPermission="recruitment_manage">
            <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-4 gap-4 mb-6">
                    ...
                    <Card>
                        <CardBody className="text-center">
                            <p className="text-2xl font-bold">{filteredJobs.reduce((acc, j) => acc + (j.candidates?.[0]?.count || 0), 0) || 0}</p>
                ...
                <Card className="mb-6">
                    <CardBody>
                        <div className="flex gap-4">
                            <Input placeholder="Rechercher par titre..." startContent={<FiSearch />} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="flex-1" />

                            <Select label="Direction" placeholder="Toutes les directions" selectedKeys={filters.direction_id ? [filters.direction_id] : []} onChange={(e) => setFilters({ ...filters, direction_id: e.target.value })} className="w-64">
                            ...
                            <Select label="Statut" placeholder="Tous les statuts" selectedKeys={filters.status ? [filters.status] : []} onChange={(e) => setFilters({ ...filters, status: e.target.value })} className="w-48">
                ...
                <Card>
                    <CardBody>
                        <Table aria-label="Postes vacants">
                            ...
                            <TableBody items={filteredJobs} isLoading={isLoading} loadingContent={<Spinner label="Chargement..." />} emptyContent="Aucun poste trouvé">
                                {(job) => (
                                    <TableRow key={job.id}>
                                        <TableCell>
                                            <div>
                                        ...
                                        <TableCell>{job.closing_date ? new Date(job.closing_date).toLocaleDateString("fr-FR") : "-"}</TableCell>
                                        <TableCell>
                                        ...
                                        <TableCell>
                                            <Dropdown>
                                                ...
                                                <DropdownMenu>
                                                    <DropdownItem key="view" startContent={<FiEye />} onPress={() => router.push(`/dashboard/recruitment/job-openings/${job.id}`)}>
                                                        Voir détails
                                                    </DropdownItem>
                                                    <DropdownItem key="edit" startContent={<FiEdit />} onPress={() => handleOpenModal(job)}>
                                                        Modifier
                                                    </DropdownItem>
                                                    {!job.is_published && job.status === "draft" && (
                                                        <DropdownItem key="publish" startContent={<FiShare2 />} onPress={() => handlePublish(job.id)}>
                                                            Publier
                                                        </DropdownItem>
                                                    )}
                                                    {job.status === "open" && (
                                                        <DropdownItem key="close" startContent={<FiLock />} onPress={() => handleClose(job.id)}>
                                                            Clôturer
                                                        </DropdownItem>
                                                    )}
                                                    <DropdownItem key="candidates" startContent={<FiUsers />} onPress={() => router.push(`/dashboard/recruitment/candidates?job=${job.id}`)}>
                                                        Voir candidatures
                                                    </DropdownItem>
                                                    {job.status === "draft" && (
                                                        <DropdownItem key="delete" startContent={<FiTrash2 />} onPress={() => handleDelete(job.id)} className="text-danger" color="danger">
                                                            Supprimer
                                                        </DropdownItem>
                                                    )}
                                                </DropdownMenu>
                    ...
                    <ModalContent>
                        {(onClose) => (
                            <form onSubmit={handleSubmit(onSubmit)}>
                                <ModalHeader>
                                    <h3 className="text-xl font-bold">{selectedJob ? "Modifier le Poste" : "Créer un Nouveau Poste"}</h3>
                                </ModalHeader>
                                <ModalBody>
                                    <div className="grid grid-cols-2 gap-4">
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Input {...field} label="Titre du poste" placeholder="Ex: Analyste Financier Senior" isRequired errorMessage={errors.title?.message} isInvalid={!!errors.title} className="col-span-2" />
                                            )}
                                        />
                                        ...
                                        <Controller
                                            ...
                                            control={control}
                                            render={({ field }) => (
                                                <Select {...field} label="Service" placeholder="Sélectionnez le service" selectedKeys={field.value ? [field.value] : []} onSelectionChange={(keys) => field.onChange(Array.from(keys)[0])}>
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => <Input {...field} type="number" label="Nombre de postes" placeholder="1" isRequired errorMessage={errors.number_of_positions?.message} isInvalid={!!errors.number_of_positions} />}
                                        />

                                        <Controller name="location" control={control} render={({ field }) => <Input {...field} label="Lieu de travail" placeholder="Ex: Kinshasa, Siège" />} />

                                        <Controller name="closing_date" control={control} render={({ field }) => <Input {...field} type="date" label="Date de clôture" />} />
                                        ...
                                        <Controller
                                            name="requirements"
                                            control={control}
                                            render={({ field }) => <Textarea {...field} label="Exigences" placeholder="Listez les exigences et qualifications requises..." minRows={4} className="col-span-2" />}
                                        />

                                        <Controller
                                            name="responsibilities"
                                            control={control}
                                            render={({ field }) => <Textarea {...field} label="Responsabilités" placeholder="Listez les principales responsabilités du poste..." minRows={4} className="col-span-2" />}
                                        />
                                    </div>
                                </ModalBody>
                                <ModalFooter>
                                    ...
                                    <Button color="danger" type="submit" isLoading={createJobMutation.isPending || updateJobMutation.isPending}>
