...
import { FiPlus, FiSearch, FiMoreVertical, FiEye, FiEdit, FiCheckCircle, FiX, FiSend, FiTrash2 } from "react-icons/fi";
import { useForm, Controller } from "react-hook-form";
import { toast } from "react-toastify";
import { useGetRecruitmentPlans, useCreateRecruitmentPlan, useUpdateRecruitmentPlan, useSubmitRecruitmentPlan, useApproveRecruitmentPlan, useAddPositionToPlan, useDeletePlanPosition } from "@/src/hooks/useRecruitment";
...
export default function RecruitmentPlanningPage() {
    ...
    const { data: dataPlans, isLoading } = useGetRecruitmentPlans(filters);
    ...
    const { control: approveControl, handleSubmit: handleApproveSubmit, reset: resetApprove, watch: watchApprove } = useForm();
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-gray-500">Gestion des besoins annuels de recrutement par direction</p>
                </div>
            ...
            <Card className="shadow-none">
                <CardBody>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <Input type="number" label="Année" placeholder="2025" value={filters.year || ""} onChange={(e) => setFilters({ ...filters, year: e.target.value })} />
                        <Select label="Direction" placeholder="Toutes" onChange={(e) => setFilters({ ...filters, direction_id: e.target.value })}>
                ...
                <ModalContent>
                    <form onSubmit={handleCreateSubmit(onCreatePlan)}>
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <Controller
                                        ...
                                        render={({ field }) => <Input {...field} type="number" label="Année" placeholder="2025" isRequired isInvalid={!!createErrors.year} errorMessage={createErrors.year?.message} />}
                                    />
                                    <Controller
                                        name="direction_id"
                                        control={createControl}
                                        ...
                                        render={({ field }) => (
                                            <Select {...field} label="Direction" placeholder="Sélectionnez une direction" isRequired isInvalid={!!createErrors.direction_id} errorMessage={createErrors.direction_id?.message}>
                                                {directions?.map((direction) => (
                                                    <SelectItem key={direction.id} value={direction.id}>
                                                        {direction.name}
                                                    </SelectItem>
                                ...
                                <Controller
                                    name="description"
                                    control={createControl}
                                    render={({ field }) => <Textarea {...field} label="Description / Objectifs" placeholder="Décrivez les objectifs et besoins de recrutement pour cette année..." minRows={3} />}
                                />
                        ...
                        <ModalFooter>
                            ...
                            <Button color="primary" type="submit" isLoading={createPlanMutation.isPending}>
                                Créer
                            </Button>
                        </ModalFooter>
                    </form>
            ...
            {/* Add Position Modal */}
            <Modal isOpen={isAddPositionOpen} onClose={onAddPositionClose} size="3xl" scrollBehavior="inside">
                <ModalContent>
                    <form onSubmit={handlePositionSubmit(onAddPosition)}>
                        ...
                        <ModalBody>
                            <div className="grid grid-cols-2 gap-4">
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Select {...field} label="Poste" placeholder="Sélectionnez un poste" isInvalid={!!positionErrors.job_position_id} errorMessage={positionErrors.job_position_id?.message}>
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        <Select {...field} label="Grade" placeholder="Sélectionnez un grade" isInvalid={!!positionErrors.grade_id} errorMessage={positionErrors.grade_id?.message}>
                                            {grades?.map((grade) => (
                                ...
                                <Controller
                                    ...
                                    control={positionControl}
                                    render={({ field }) => (
                                        <Select {...field} label="Service" placeholder="Sélectionnez un service">
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => <Input {...field} type="number" label="Quantité" placeholder="1" isInvalid={!!positionErrors.quantity_needed} errorMessage={positionErrors.quantity_needed?.message} />}
                                />
                                <Controller
                                    name="priority"
                                    control={positionControl}
                                    ...
                                    render={({ field }) => (
                                        <Select {...field} label="Priorité" placeholder="Sélectionnez" isInvalid={!!positionErrors.priority} errorMessage={positionErrors.priority?.message}>
                                ...
                                <Controller name="budget_allocated" control={positionControl} render={({ field }) => <Input {...field} type="number" label="Budget Alloué (CFD)" placeholder="0" />} />
                        ...
                        <ModalFooter>
                            ...
                            <Button color="primary" type="submit" isLoading={addPositionMutation.isPending}>
                                Ajouter
                            </Button>
                        </ModalFooter>
                    </form>
                        ...
                        <ModalBody>
                            <div className="space-y-4">
                                ...
                                {watchApprove("approve") === false && (
                                    <Controller name="rejection_reason" control={approveControl} render={({ field }) => <Textarea {...field} label="Raison du rejet" placeholder="Expliquez pourquoi vous rejetez ce plan..." rows={4} />} />
                                )}
                            </div>
                        </ModalBody>
                        <ModalFooter>
                            ...
                            <Button color={watchApprove("approve") === false ? "danger" : "success"} type="submit" isLoading={approvePlanMutation.isPending}>
                                Confirmer
                            </Button>
                        </ModalFooter>
                    </form>
            ...
            {selectedPlanDetails && (
                <Modal isOpen={isDetailOpen} onClose={onDetailClose} size="5xl" scrollBehavior="inside">
                    <ModalContent>
                        <ModalHeader>Détails du Plan - {selectedPlanDetails.year}</ModalHeader>
                        <ModalBody>
                            <div className="space-y-6">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                    ...
                                    <div>
                                        ...
                                        <p className="font-semibold">{selectedPlanDetails.direction?.name}</p>
                                    </div>
                                    <div>
                                        <p className="text-sm text-gray-500">Statut</p>
                                        <Chip color={getStatusColor(selectedPlanDetails.status)} variant="flat">
                                            {getStatusLabel(selectedPlanDetails.status)}
                                        </Chip>
                                    </div>
                                    <div>
                                        <p className="text-sm text-gray-500">Total Positions</p>
                                        <p className="font-semibold">{selectedPlanDetails.positions?.length || 0}</p>
                                    </div>
                                    {selectedPlanDetails.submitted_date && (
                                        <div>
                                            <p className="text-sm text-gray-500">Date de soumission</p>
                                            <p className="font-semibold">{new Date(selectedPlanDetails.submitted_date).toLocaleDateString("fr-FR")}</p>
                                        </div>
                                    )}
                                    {selectedPlanDetails.approver && (
                                        <div>
                                            <p className="text-sm text-gray-500">Approuvé par</p>
                                            <p className="font-semibold">{selectedPlanDetails.approver.username}</p>
                                        </div>
                                    )}
                                    {selectedPlanDetails.approved_date && (
                                        <div>
                                            <p className="text-sm text-gray-500">Date d'approbation</p>
                                            <p className="font-semibold">{new Date(selectedPlanDetails.approved_date).toLocaleDateString("fr-FR")}</p>
                                        </div>
                                    )}
                                    <div>
                                        <p className="text-sm text-gray-500">Date de création</p>
                                        <p className="font-semibold">{new Date(selectedPlanDetails.created_at).toLocaleDateString("fr-FR")}</p>
                                    </div>
                                </div>

                                {selectedPlanDetails.notes && (
                                    <div className="p-4 bg-gray-50 rounded-lg">
                                        <p className="text-sm font-semibold text-gray-700 mb-2">Description / Objectifs:</p>
                                        <p className="text-sm text-gray-600">{selectedPlanDetails.notes}</p>
                                    </div>
                                )}

                                {selectedPlanDetails.rejection_reason && (
                                    <div className="p-4 bg-danger-50 rounded-lg">
                                        <p className="text-sm text-danger-600 font-semibold">Raison du rejet:</p>
                                        <p className="text-sm mt-2">{selectedPlanDetails.rejection_reason}</p>
                                    </div>
                                )}
