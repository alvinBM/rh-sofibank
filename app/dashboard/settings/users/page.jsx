        ...
        return (
          <div className="flex gap-1 flex-wrap">
            {user.roles?.slice(0, 2).map((ur) => (
              <Chip key={ur.role.id} size="sm" variant="flat" color="primary">
                {ur.role.name}
              </Chip>
            ))}
            {user.roles?.length > 2 && (
        ...
        <Modal isOpen={isRolesOpen} onClose={onRolesClose} size="2xl">
          <ModalContent>
            {(onClose) => (
              <>
                <ModalHeader>
                  Gérer les rôles - {selectedUser?.full_name}
                </ModalHeader>
                <ModalBody>
                      ...
                      return (
                        <div key={role.id} className="flex justify-between items-center p-3 border rounded-lg">
                          ...
                          <Switch
                            ...
                            onValueChange={() => handleToggleRole(role.id, isAssigned)}
                          />
                        </div>
                      );
                    })}
