...
import { useGetHolidays, useCreateHoliday, useUpdateHoliday, useDeleteHoliday } from "@/src/hooks/useSettings";
...
export default function HolidaysPage() {
            ...
            case "date":
                return (
                    <div className="flex flex-col">
                        <p className="text-sm font-semibold">{new Date(holiday.date).toLocaleDateString("fr-FR")}</p>
                        <p className="text-xs text-default-400">{new Date(holiday.date).toLocaleDateString("fr-FR", { weekday: "long" })}</p>
                    </div>
                );
            case "recurring":
            ...
            case "actions":
                return (
                    <Dropdown>
                        ...
                        <DropdownMenu>
                            <DropdownItem key="edit" startContent={<FiEdit />} onPress={() => handleOpenEdit(holiday)}>
                                Modifier
                            </DropdownItem>
                            <DropdownItem key="delete" className="text-danger" color="danger" startContent={<FiTrash2 />} onPress={() => handleDelete(holiday.id)}>
                                Supprimer
                            </DropdownItem>
                        </DropdownMenu>
                    </Dropdown>
    ...
    return (
        <PermissionGuard requiredPermissions={["holidays_manage", "settings_access"]}>
            <div className="p-0">
                <div className="flex flex-col gap-4">
                    ...
                    <Card>
                        <CardBody className="flex flex-row items-center gap-3">
                            <div className="p-3 rounded-lg bg-danger/10">
                                <FiCalendar className="text-danger text-2xl" />
                            </div>
                            <div>
                    ...
                    <div className="flex gap-3">
                        <Input isClearable placeholder="Rechercher un jour férié..." startContent={<FiSearch />} value={searchQuery} onValueChange={handleSearchChange} className="flex-1" />
                        <Select label="Année" selectedKeys={[selectedYear]} onChange={(e) => setSelectedYear(e.target.value)} className="w-40">
                    ...
                    >
                        ...
                        <TableBody items={holidays} isLoading={isLoading} loadingContent={<Spinner label="Chargement..." />} emptyContent="Aucun jour férié trouvé">
                            {(holiday) => <TableRow key={holiday.id}>{(columnKey) => <TableCell>{renderCell(holiday, columnKey)}</TableCell>}</TableRow>}
                        </TableBody>
                    </Table>
                                    ...
                                    <CardBody>
                                        ...
                                        <div className="space-y-2">
                                            {monthHolidays.map((holiday) => (
                                                <div key={holiday.id} className="flex justify-between items-center p-2 bg-default-100 rounded-lg">
                                                    <div>
                ...
                <Modal isOpen={isOpen} onClose={onClose} size="2xl">
                    <ModalContent>
                        {(onClose) => (
                            <>
                                <ModalHeader>{editMode ? "Modifier le jour férié" : "Nouveau jour férié"}</ModalHeader>
                                <ModalBody>
                                    <div className="flex flex-col gap-4">
                                        <Input label="Nom du jour férié" placeholder="Ex: Fête de l'Indépendance" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} isRequired />
                                        <Input label="Date" type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} isRequired />
                                        <Switch isSelected={formData.is_recurring} onValueChange={(value) => setFormData({ ...formData, is_recurring: value })}>
                                            Jour férié récurrent (se répète chaque année)
                                        </Switch>
                                        <Input label="Description" placeholder="Description ou note (optionnelle)" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                                    </div>
                                </ModalBody>
                                <ModalFooter>
                                    ...
                                    <Button color="danger" onPress={handleSubmit} isLoading={createHolidayMutation.isLoading || updateHolidayMutation.isLoading}>
